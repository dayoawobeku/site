...
async function getPost(slug: string) {
    const res = await fetch(`${site}/wp-json/wp/v2/posts?slug=${slug.replace('/blog/', '')}&per_page=1&_embed`, { next: { revalidate: 10 } })
...
async function generateMetadata({ params, searchParams }: { params: any, searchParams: any }) {
    ...
    console.log(post);
    return {
        title: `${post[0].title.rendered} | David M. Coleman`
    };
}
...
async function BlogPost({ params }: {
    ...
    return (
        <>
           {data.map((post: any) => {
                const postDate = format( new Date( post.date_gmt ), 'EEEE, MMMM do, yyyy' );
                        ...
                        <div className="xl:divide-y xl:divide-gray-200 xl:dark:divide-gray-700">
                            <header className="pt-6 xl:pb-5">
                                <div className="space-y-1 text-center">
                                    ...
                                    <div>
                                        <h1 className="text-3xl font-extrabold leading-9 tracking-tight text-gray-900 dark:text-gray-100 sm:text-4xl sm:leading-10 md:text-5xl md:leading-14" dangerouslySetInnerHTML={{__html: post.title.rendered}}></h1>
                                    </div>
                                    <div className="flex justify-center gap-5 py-4">
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="h-5 w-5 fill-current" height="1em" width="1em"><path d="m15.7 3.4.8-.7a3 3 0 0 1 3.3 0l.8.7.7.8a3 3 0 0 1 0 3.4l-.7.8L9.8 19.2a5 5 0 0 1-.7.6l-.8.5-1 .4c-1.3.5-2.2 1-2.8 1-.8.2-1.3 0-1.7-.4-.4-.4-.6-1-.5-1.8.1-.6.5-1.5 1-2.8l.5-1 .4-.7.6-.7L15.7 3.4Zm-2 4.9-7.5 7.4-.3.4-.3.5-.4 1-.9 2v.1c.6-.1 1.3-.5 2.2-.9l1-.4.5-.2.4-.4 7.4-7.4-2.2-2.1Zm3.5.7 2-2 .5-.5a1 1 0 0 0 0-1.2l-.5-.5-.5-.4a1 1 0 0 0-1.1 0c-.2 0-.3.2-.5.4l-2 2L17 9Z"/></svg>
                                            {post.read_time.words.toLocaleString()} words
                                        </span>
                                        <span className="flex items-center gap-1.5">
                                            ...
                                            {post.read_time.time.minutes.toLocaleString()} min read
                                        </span>
                                        <span className="flex items-center gap-1.5">
                            ...
                            <div className="divide-y divide-gray-200 pb-8 dark:divide-gray-700 xl:grid xl:grid-cols-4 xl:gap-x-6 xl:divide-y-0" style={{gridTemplateRows: "auto 1fr"}}>
                                ...
                                <div className="divide-y divide-gray-200 dark:divide-gray-700 xl:col-span-3 xl:row-span-2 xl:pb-0">
                                    <div className="prose max-w-none pt-10 pb-8 dark:prose-dark" dangerouslySetInnerHTML={{__html: post.content.rendered}}></div>
                                </div>
                                <footer>
